...
# Generate the inputs for the nearest neighboor first cut matching with the length of 9 years.
######################################################################################################
...
target_data.to_csv("/Users/dorh012/Documents/2021/stitches/notebooks/stitches_dev/inputs/target_data_9yrs.csv", index = False)
...
final.to_csv("/Users/dorh012/Documents/2021/stitches/notebooks/stitches_dev/inputs/archive_data_9yrs.csv", index = False)


# Generate the inputs for the nearest neighboor first cut matching with the length of 5 years.
######################################################################################################
# Subset the data so that it contains a single realization of a middle of the road
# scenario for a model. This is going to be the time series we want to emulate aka
# our the target_smoothed_tgav_timeseries from (https://gcims.atlassian.net/wiki/spaces/HOME/pages/669220883/stitches+design+document)
...
# This is our target data that we will use in the nearest neighbor to match on
target_data = matchup.get_chunk_info(matchup.chunk_ts(target_smooth, 5))
target_data.to_csv("/Users/dorh012/Documents/2021/stitches/notebooks/stitches_dev/inputs/target_data_5yrs.csv", index = False)
...
out = []
for key, d in data.groupby(group_by):
    rslt = matchup.get_chunk_info(matchup.chunk_ts(df=d, n=5))
    out.append(rslt)
# end of the for loop

final = pd.concat(out, ignore_index=True)
final.to_csv("/Users/dorh012/Documents/2021/stitches/notebooks/stitches_dev/inputs/archive_data_5yrs.csv", index = False)



# Save copies of the raw-data that will be stitched together and also copies of the comparison data
# to be used in the validation tests.
######################################################################################################
