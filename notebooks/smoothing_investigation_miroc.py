...
smoothed5 = smooth_and_tidy(t_anomaly, 5)
smoothed11 = smooth_and_tidy(t_anomaly, 11)
smoothed13 = smooth_and_tidy(t_anomaly, 13)
...
# concatenate it all
smoothed = pd.concat([smoothed0, smoothed5,
                      smoothed11,
                      smoothed13, smoothed19,
...
single2 = single.loc[(single['window'] == '0') | (single['window'] == '5') | (single['window'] == '13')].copy()
...
# Filter to one single ensemble member for some plots digging into
# smoothing
window13 = smoothed[smoothed['window'] == '13'].copy()
p6 = sns.relplot(data=window13, x='year', y='value',
                 hue='ensemble',
                 col='experiment',
                 kind='line',
                 linewidth=0.6)
p6.fig.subplots_adjust(top=0.85)
p6.fig.suptitle(ESM + " all ensemble members, smoothing = 13")
p6.savefig('./figs/' + ESM + '/' + ESM + '_time_series_all_ensemble_members13.png')


window5 = smoothed[smoothed['window'] == '5'].copy()
p7 = sns.relplot(data=window5, x='year', y='value',
                 hue='ensemble',
                 col='experiment',
                 kind='line',
                 linewidth=0.6)
p7.fig.subplots_adjust(top=0.85)
p7.fig.suptitle(ESM + " all ensemble members, smoothing = 5")
p7.savefig('./figs/' + ESM + '/' + ESM + '_time_series_all_ensemble_members5.png')
