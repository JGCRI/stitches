...
single2 = single.loc[(single['window'] == '0') | (single['window'] == '9') | (single['window'] == '15')].copy()
...
# Filter to one single ensemble member for some plots digging into
# smoothing
window15 = smoothed[smoothed['window'] == '15'].copy()
p6 = sns.relplot(data=window15, x='year', y='value',
                 hue='ensemble',
                 col='experiment',
                 kind='line',
                 linewidth=0.6)
p6.fig.subplots_adjust(top=0.85)
p6.fig.suptitle(ESM + " all ensemble members, smoothing = 15")
p6.savefig('./figs/' + ESM + '/' + ESM + '_time_series_all_ensemble_members15.png')
