...
Generate the recipe for the gridded archive using functions defined in gridded_recipe.R that is a essentially a wrapper of the code above ^^^^ + some clean up that is needed (discovered via trial and error). 

```{r}
gridded_recipe <- generate_gridded_recipe(target_data = target_data, archive_data = archive_subset, n = 4)
write.csv(gridded_recipe, file = "recipes_for_python.csv", row.names = FALSE)
```
