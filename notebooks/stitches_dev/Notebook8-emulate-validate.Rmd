...
for (i in 1:length(files)){
  ...
  rm(tmp)
   ...
   write.csv(plotting_tbl ,
           paste0('generated_ensembles/', esm_name ,'/metrics_', esm_experiment,'_draw', unique(plotting_tbl$draw), '.csv'), 
            row.names = FALSE)
}
...
For now, we do a random sample of a generated ensemble member and a target ensemble member to compare the plots of their ACF and partial ACF functions of our Tgav time series (not jumps). For now, we settle on visual inspection to detect if they're similar and if there are any odd 'happenings' every 9 years (the window size) in the generated ACF. 

```{r}
# List the IDs of all the generated and real ensemble members for sampling
generated_tgavs %>%
  filter(archive == paste0('without_', esm_experiment) ) %>%
  select(stitching_id, tol, archive, draw) %>%
  distinct  ->
  gen_ids_wo

generated_tgavs %>%
  filter(archive == paste0('with_', esm_experiment) ) %>%
  select(stitching_id, tol, archive, draw) %>%
  distinct  ->
  gen_ids_w
...
# Randomly sample one of the generated Tgavs from the archive without target experiment
random_gen_id_wo <- gen_ids_wo[sample(nrow(gen_ids_wo), 1),]
generated_tgavs %>%
  filter(stitching_id == random_gen_id_wo$stitching_id,
         tol == random_gen_id_wo$tol,
         archive == random_gen_id_wo$archive,
         draw == random_gen_id_wo$draw) ->
  gen_random_wo

random_gen_id_w <- gen_ids_w[sample(nrow(gen_ids_w), 1),]
generated_tgavs %>%
  filter(stitching_id == random_gen_id_w$stitching_id,
         tol == random_gen_id_w$tol,
         archive == random_gen_id_w$archive,
         draw == random_gen_id_w$draw) ->
  gen_random_w

# Randomly sample one of the target Tgavs
random_targ_id <- target_ids[sample(nrow(target_ids), 1),]
original_data %>%
  filter(experiment == random_targ_id$experiment,
         ensemble == random_targ_id$ensemble) ->
  targ_random


# Compare ACF and partial ACF plots
par(mfrow=c(3,2))
acf(targ_random$value,
    main= paste("Target Ensemble member:", random_targ_id$experiment, random_targ_id$ensemble))
acf(targ_random$value, type='partial',
     main= paste("Target Ensemble member:", random_targ_id$experiment, random_targ_id$ensemble))
acf(gen_random_wo$value,
     main= paste("Generated:", random_gen_id_wo$stitching_id, '\n',
                'tol=', random_gen_id_wo$tol, '. archive: ', random_gen_id_wo$archive, '. draw =', random_gen_id_wo$draw))
acf(gen_random_wo$value, type='partial',
    main= paste("Generated:", random_gen_id_wo$stitching_id, '\n',
                'tol=', random_gen_id_wo$tol, '. archive: ', random_gen_id_wo$archive, '. draw =', random_gen_id_wo$draw))
acf(gen_random_w$value,
     main= paste("Generated:", random_gen_id_w$stitching_id, '\n',
                'tol=', random_gen_id_w$tol, '. archive:', random_gen_id_w$archive, '. draw =', random_gen_id_w$draw))
acf(gen_random_w$value, type='partial',
     main= paste("Generated:", random_gen_id_w$stitching_id, '\n',
                'tol=', random_gen_id_w$tol, '. archive: ', random_gen_id_w$archive, '. draw =', random_gen_id_w$draw))
par(mfrow=c(1,1))
```
