...
def make_recpies(target_data, archive_data, tol, N_matches, res, variables=None):
    """ Make recipes to use in the stitching process.

         :param target_data:    pandas dataframe of the data to emulate
         :archive_data:         pandas dataframe of ESM to match
         :tol:                  double tolerance for the matching
         :N_matches:            int max number of matches per target ts
         :res:                  string mon or day to indicate the resolution of the data
         :variables:            set of variable strings

         :return:                   a recipe data frame
     """


    util.check_columns(target_data, {'experiment', 'variable', 'ensemble', 'model', 'start_yr',
                                     'end_yr', 'year', 'fx', 'dx'})
    util.check_columns(target_data, {'experiment', 'variable', 'ensemble', 'model', 'start_yr',
                                     'end_yr', 'year', 'fx', 'dx'})
    ...
    # Import the pangeo table this will be used to help
    ptable_path = pkg_resources.resource_filename('stitches', 'data/pangeo_table.csv')
    pangeo_table = pd.read_csv(ptable_path)

    # Subset the pangeo table so that is only has the resolution of data we are
    # interested in!
    if res == 'mon':
        table = 'Amon'
    else:
        table = 'day'
    # Update the pangeo table so that it only has data of the resolution of interest.
    pangeo_table = pangeo_table[(pangeo_table['domain'] == table)].copy()
    pvars = set(pangeo_table['variable'].unique())

    if not(variables is None or type(variables) is set):
        raise TypeError(f"variables must be None or a set")
    if type(variables) is set:
        if 'tas' in variables:
            raise TypeError(f"variables only includes cmip strings for non tas variables")
        if not(variables.issubset(pvars)):
            raise TypeError(f"variable name not recognized")
            # Check the strings entered into the variables set the variable names must match the cmip
            # variable names. Otherwise it could be that the variable requested is not available for the
            # resolution/model selected.

            # Update the archive data so that it is limited to the tas values that pair with the other non
            # tas variables.
            data_avail = (pangeo_table[(pangeo_table['variable'].isin(list(variables)))][['model', 'experiment',
                                                                                          'ensemble', 'variable']]
                          .drop_duplicates()
                          .reset_index(drop=True)
                          .copy())
            data_avail['exists'] = 1
            to_keep = (data_avail.pivot_table(index=["model", "experiment", "ensemble"],
                                              columns="variable",
                                              values="exists").reset_index().dropna().copy()[
                ['model', 'experiment', 'ensemble']])
            archive_data = pd.merge(archive_data, to_keep, on=['model', 'experiment', 'ensemble']).copy()

            if util.nrow(archive_data) <= 0:
                raise TypeError(f"insufficient data available for tas & variables of interest")
    ...
    # Get recipes based on the number of matches.
    unformatted_recipe = permute_stitching_recipes(N_matches=N_matches, matched_data=match_df, archive=archive_data)

    # Format the recipe so that is it meets the requirements to be stitched, aka each realization must be unique
    # series of tas data with zstore files that actually match the pangeo files (there should be no issues
    # related to the transition from historical to future period).
    ...
    if type(variables) is set:
        # If applicable add the zstore information for the non tas files
        zstore_table = (pangeo_table.loc[(pangeo_table['variable'].isin(list(variables)))][['model', 'experiment',
                                                                                            'ensemble', 'variable',
                                                                                            'zstore']]
                        .reset_index(drop=True)
                        .drop_duplicates())
        zstore_table['variable'] = zstore_table['variable'] + "_file"
        zstore_table = (zstore_table.pivot(index=['model', 'experiment', 'ensemble'],
                                           columns='variable', values='zstore').reset_index().copy())
        zstore_table = zstore_table.rename(columns={'model': 'archive_model',
                                                    'experiment': 'archive_experiment',
                                                    'ensemble': 'archive_ensemble'})
        recipe = recipe.merge(zstore_table)
