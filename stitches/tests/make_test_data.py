...
match_df = match_neighborhood(target_data, archive_data, tol=0.1, drop_hist_duplicates=False)
match_path = pkg_resources.resource_filename('stitches', 'tests/test-match_w_dup.csv')
match_df.to_csv(match_path, index=False)

match_df = match_neighborhood(target_data, archive_data, tol=0.1, drop_hist_duplicates=True)
match_path = pkg_resources.resource_filename('stitches', 'tests/test-match.csv')
match_df.to_csv(match_path, index=False)
...
match_path = pkg_resources.resource_filename('stitches', 'tests/test-match_dat.csv')
match_df.to_csv(match_path, index=False)